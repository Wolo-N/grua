...
def optimize_member_sizes_simple():
    '''Simple optimization: optimize only member cross-sections for fixed geometry'''
    from scipy.optimize import minimize

    print("\n" + "="*70)
    print("GRUITA 3.0 - SELF-OPTIMIZING CRANE (Simple Version)")
    print("="*70)
    print("Optimizing member sizes to minimize cost while maintaining safety")
    print("Fixed geometry, varying cross-sectional areas only")
    print("="*70)

    # Fixed geometry from gruita2
    X, tower_base, boom_top_nodes, boom_bot_nodes = design_crane_geometry()
    C = create_crane_connectivity(tower_base, boom_top_nodes, boom_bot_nodes)
    ...
    history = {'areas': [], 'cost': [], 'FS_t': [], 'FS_p': [], 'penalty': []}

    def evaluate_member_sizes(areas):
        '''Evaluate design with given member areas'''
        A_main, A_secondary = areas
        ...
        cost = calculate_cost(mass, C.shape[0], X.shape[0])
        ...
        max_compressed_length = max([np.linalg.norm(X[C[iEl,1]] - X[C[iEl,0]])
                                      for iEl in range(C.shape[0]) if element_forces[iEl] < 0] or [1])
        P_critica = (np.pi**2 * E * I_min) / (max_compressed_length**2)

        FS_tension, FS_pandeo = calculate_security_factors(sigma_max, P_max, 100e6, P_critica)

        return cost, FS_tension, FS_pandeo

    def objective(areas):
        '''Minimize cost with penalties for safety violations'''
        cost, FS_t, FS_p = evaluate_member_sizes(areas)

        penalty = 0
        if FS_t < min_FS:
            penalty += 1000 * (min_FS - FS_t)**2
        if FS_p < min_FS:
            penalty += 1000 * (min_FS - FS_p)**2

        # Store history
        history['areas'].append(areas.copy())
        history['cost'].append(cost)
        history['FS_t'].append(FS_t)
        history['FS_p'].append(FS_p)
        history['penalty'].append(penalty)

        print(f"  A_main={areas[0]*10000:.1f}cm², A_sec={areas[1]*10000:.1f}cm² → Cost={cost:.4f}, FS_t={FS_t:.2f}, FS_p={FS_p:.2f}")
        return cost + penalty

    # Initial guess
    x0 = [0.01, 0.005]
    bounds = [(0.003, 0.025), (0.002, 0.015)]

    print("\nOptimizing...")
    result = minimize(objective, x0, method='L-BFGS-B', bounds=bounds,
                     options={'maxiter': 30, 'disp': True})

    A_main_opt, A_sec_opt = result.x
    cost_opt, FS_t_opt, FS_p_opt = evaluate_member_sizes(result.x)

    print("\n" + "="*70)
    print("OPTIMIZATION RESULTS")
    print("="*70)
    print(f"Optimal Main Area:      {A_main_opt*10000:.2f} cm²")
    print(f"Optimal Secondary Area: {A_sec_opt*10000:.2f} cm²")
    print(f"Final Cost:             {cost_opt:.4f}")
    print(f"FS_tension:             {FS_t_opt:.2f}")
    print(f"FS_pandeo:              {FS_p_opt:.2f}")
    print("="*70)

    # Compare with baseline
    cost_base, FS_t_base, FS_p_base = evaluate_member_sizes([0.01, 0.005])
    print(f"\nBaseline (10cm², 5cm²): Cost={cost_base:.4f}, FS_t={FS_t_base:.2f}, FS_p={FS_p_base:.2f}")
    if cost_opt < cost_base:
        print(f"Improvement: {((cost_base-cost_opt)/cost_base*100):.1f}% cost reduction")
    else:
        print(f"Note: Cost increased {((cost_opt-cost_base)/cost_base*100):.1f}% to meet safety requirements")
    print("="*70)
    ...
    ax1.plot(iterations, history['cost'], 'b-o', linewidth=2, markersize=6)
    ax1.axhline(y=cost_base, color='r', linestyle='--', label='Baseline cost')
    ax1.set_xlabel('Iteration', fontsize=11)
    ax1.set_ylabel('Cost', fontsize=11)
    ax1.set_title('Cost Evolution', fontsize=13, fontweight='bold')
    ax1.legend()
    ax1.grid(True, alpha=0.3)

    # Plot 2: Safety factors evolution
    ax2 = plt.subplot(3, 2, 2)
    ax2.plot(iterations, history['FS_t'], 'g-o', linewidth=2, markersize=6, label='FS_tension')
    ax2.plot(iterations, history['FS_p'], 'r-o', linewidth=2, markersize=6, label='FS_pandeo')
    ax2.axhline(y=min_FS, color='k', linestyle='--', linewidth=2, label=f'Min FS = {min_FS}')
    ax2.set_xlabel('Iteration', fontsize=11)
    ax2.set_ylabel('Safety Factor', fontsize=11)
    ax2.set_title('Safety Factors Evolution', fontsize=13, fontweight='bold')
    ax2.legend()
    ax2.grid(True, alpha=0.3)

    # Plot 3: Member areas evolution
    ax3 = plt.subplot(3, 2, 3)
    areas_main = [a[0]*10000 for a in history['areas']]
    areas_sec = [a[1]*10000 for a in history['areas']]
    ax3.plot(iterations, areas_main, 'b-o', linewidth=2, markersize=6, label='Main members')
    ax3.plot(iterations, areas_sec, 'r-o', linewidth=2, markersize=6, label='Secondary members')
    ax3.axhline(y=100, color='b', linestyle='--', alpha=0.5, label='Baseline main')
    ax3.axhline(y=50, color='r', linestyle='--', alpha=0.5, label='Baseline secondary')
    ax3.set_xlabel('Iteration', fontsize=11)
    ax3.set_ylabel('Cross-sectional Area (cm²)', fontsize=11)
    ax3.set_title('Member Sizes Evolution', fontsize=13, fontweight='bold')
    ax3.legend()
    ax3.grid(True, alpha=0.3)

    # Plot 4: Penalty evolution
    ax4 = plt.subplot(3, 2, 4)
    ax4.plot(iterations, history['penalty'], 'purple', linewidth=2, marker='o', markersize=6)
    ax4.set_xlabel('Iteration', fontsize=11)
    ax4.set_ylabel('Penalty (constraint violations)', fontsize=11)
    ax4.set_title('Penalty Evolution', fontsize=13, fontweight='bold')
    ax4.grid(True, alpha=0.3)

    # Plot 5: Comparison bar chart
    ax5 = plt.subplot(3, 2, 5)
    categories = ['Main Area\n(cm²)', 'Secondary Area\n(cm²)', 'Cost', 'FS_tension', 'FS_pandeo']
    baseline_vals = [100, 50, cost_base, FS_t_base, FS_p_base]
    optimized_vals = [A_main_opt*10000, A_sec_opt*10000, cost_opt, FS_t_opt, FS_p_opt]

    x_pos = np.arange(len(categories))
    width = 0.35

    bars1 = ax5.bar(x_pos - width/2, baseline_vals, width, label='Baseline', color='lightcoral')
    bars2 = ax5.bar(x_pos + width/2, optimized_vals, width, label='Optimized', color='lightgreen')

    ax5.set_ylabel('Value', fontsize=11)
    ax5.set_title('Baseline vs Optimized Comparison', fontsize=13, fontweight='bold')
    ...
    sc = ax6.scatter([a[0]*10000 for a in history['areas']],
                     [a[1]*10000 for a in history['areas']],
                     c=history['cost'], cmap='viridis', s=100, alpha=0.7, edgecolors='black')
    ax6.plot(100, 50, 'r*', markersize=20, label='Baseline')
    ax6.plot(A_main_opt*10000, A_sec_opt*10000, 'g*', markersize=20, label='Optimized')

    # Add arrows showing progression
    for i in range(len(history['areas'])-1):
        ax6.annotate('', xy=(history['areas'][i+1][0]*10000, history['areas'][i+1][1]*10000),
                    xytext=(history['areas'][i][0]*10000, history['areas'][i][1]*10000),
                    arrowprops=dict(arrowstyle='->', color='gray', alpha=0.5, lw=1.5))

    ax6.set_xlabel('Main Member Area (cm²)', fontsize=11)
    ax6.set_ylabel('Secondary Member Area (cm²)', fontsize=11)
    ax6.set_title('Design Space Exploration', fontsize=13, fontweight='bold')
    ax6.legend()
    ...
    # Plot optimized crane structure
    plot_truss(X, C, f"Optimized Crane - Main:{A_main_opt*10000:.1f}cm², Sec:{A_sec_opt*10000:.1f}cm²", show_nodes=True)

    return result.x, cost_opt

if __name__ == "__main__":
    print("="*70)
    print("GRUITA 3.0 - SELF-OPTIMIZING CRANE")
    print("="*70)

    # Run simple optimization
    optimize_member_sizes_simple()